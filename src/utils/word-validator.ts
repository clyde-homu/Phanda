...
const VALID_WORDS = new Set([
  // 3-letter words
  'CAT', 'CAR', 'ART', 'TAR', 'RAT', 'ACT',
  'SUN', 'RUN', 'NUN', 'SON', 'WON', 'ONE',
  'ORE', 'ROE', 'ERA', 'EAR', 'ARE',
  
  // 4-letter words
  'CART', 'CARD', 'CARE', 'RACE', 'ACRE', 'ARCH',
  'LOVE', 'OVER', 'ROVE', 'LORE', 'ROLE', 'VOLE',
  'WORN', 'NORM', 'FORM', 'ROOM', 'MOON', 'SOON',
  
  // 5-letter words
  'LOVER', 'ROVER', 'COVER', 'CLOVER',
  'MOURN', 'ROUND', 'SOUND', 'POUND', 'BOUND',
  'GREAT', 'TREAT', 'WHEAT', 'CHEAT', 'HEART',
  
  // 6-letter words
  'WONDER', 'WANDER', 'TENDER', 'RENDER', 'SENDER',
  'MASTER', 'MISTER', 'SISTER', 'LISTER', 'FISTER',
  
  // 7-letter words
  'AMAZING', 'AWESOME', 'SPECIAL', 'MAGICAL', 'CAREFUL'
])
...
export const areLettersAdjacent = (index1: number, index2: number, totalLetters: number): boolean => {
  // For circular arrangement, check if indices are adjacent
  const diff = Math.abs(index1 - index2)
  return diff === 1 || diff === totalLetters - 1
}

export const canConnectLetters = (currentIndices: number[], newIndex: number, totalLetters: number): boolean => {
  ...
  return isWithinConnectionRange(lastIndex, newIndex, totalLetters) && !currentIndices.includes(newIndex)
}

export const isWithinConnectionRange = (index1: number, index2: number, totalLetters: number): boolean => {
